...
load("C:/Users/aleav/Dropbox/Bayesian Graphical Models/BMIM/phd-Andrea/Data/appl10paper_november.RData")
...
dataX[[1]] <-as.matrix(datax[which(datax$profile=="0"),-11])
dataX[[2]] <-as.matrix(datax[which(datax$profile=="1"),-11])
dataX[[3]] <-as.matrix(datax[which(datax$profile=="2"),-11])
...
################################### end dssl
  ...
  sh_mat.10.BAL<-sh_mat
  
  theta.BAL<-thetaK
  
  meanssssum.10.BAL<-meanssssum
...
for(x in 1:X){
  ...
  sssum<-apply(PREDBEL[[x]][[P]][4001:5000,],2,sum)/1000
  ...
  for (i in 1:X){
    ...
    for (j in 1:X){
      ...
      if (i==j){
        ...
        sh_mat[i,j]<-(length(which(TEST[[i]]!=0)))/2
        
        
      } else if (i<j) {
        
        
        sh_mat[i,j]<-(length(which(TEST[[i]]+TEST[[j]]==2 | TEST[[i]]+TEST[[j]]==200)))/2
  ...
  sh_mat.10.BEL<-sh_mat
  
  theta.BEL<-thetaK
  
  meanssssum.10.BEL<-meanssssum
...
for(x in 1:X){
  ...
  sssum<-apply(PREDBE[[x]][[P]][4001:5000,],2,sum)/1000
